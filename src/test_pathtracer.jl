...
function render(sirina, visina)
    ...
    light = Sphere(1.0, 2.0, -1.5, 1.5, 0.0, RGB{N0f8}(0,0,0);light_source=true)
    ...
    red_sphere   = Sphere(0.0, 0.0 , 2.0, 1.0, 32.0, RGB{N0f8}(1.0, 0.0, 0.0))   
    ...
    for j in 1:cam.ny, i in 1:cam.nx
        ...
        for k in 1:5
            color += RGB{Float64}(pathtrace(ray, objects, 3, ambient, 0.5; max_inc=10))
        end
        img[i, j] = color / 5
    end
    return img
